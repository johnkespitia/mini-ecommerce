<script>
	$(document).ready(function() {
		if ($("#relationship").val() != "CONVENIO EMPRESARIAL") {
	...
	<div class="card-body">
		...
		<form method="post" action="/car/update/<?= $customer["id"] ?>">
			...
			<div class="form-group">
				...
				<select required class="form-control" name="relationship" id="relationship" onchange="showConvenios(this)">
			...
			<div class="form-group" id="ConvenioEMpresarialfield">
				...
				<select class="form-control" id="ConvenioEMpresarial" name="company_agreement">
					...
					foreach ($companyAgreementList as $ct) {
						$selected = ($customer["company_agreement"] == $ct["id"]) ? "selected='selected'" : "";
						echo "<option {$selected} value='{$ct["id"]}'>{$ct["name"]}</option>";
					}
					?>
				</select>
			...
			<div class="form-group">
				...
				<input type="number" min="0" class="form-control" name="oil_change_km" value="<?= $customer["oil_change_km"] ?>" />
			</div>
			<div class="form-group">
